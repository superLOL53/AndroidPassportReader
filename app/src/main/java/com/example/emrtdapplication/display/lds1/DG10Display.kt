...
object DG10Display : CreateView() {
    ...
    override fun <T : LinearLayout> createView(context: Context, parent: T) {
        if (EMRTD.ldS1Application.dg10.rawFileContent == null) return
        if (EMRTD.showDetails) {
            ...
            text.text = context.getString(R.string.dg10_file_content)
            parent.addView(text)
            ...
            text.text = EMRTD.ldS1Application.dg10.rawFileContent?.toHexString(HexFormat { upperCase=true; bytes.byteSeparator = " "})
        }
    }
}