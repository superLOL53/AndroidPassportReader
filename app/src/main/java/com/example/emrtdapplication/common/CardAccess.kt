 ...
 * @property paceDomainParams Array list of
 * [PACEDomainParameterInfo] contained in the file
 * @throws IllegalArgumentException If the file does
 * not contain [PACEInfo] or [PACEDomainParameterInfo]
 */
class CardAccess {
    ...
    fun read(): Int {
        var info = APDUControl.sendAPDU(
            APDU(
                ...
                byteArrayOf(CARD_ACCESS_ID_1, CARD_ACCESS_ID_2)
            )
        )
     ...
     * @throws IllegalArgumentException If [b] does not contain
     * [PACEInfo] or [PACEDomainParameterInfo]
     */
    fun parse(b: ByteArray): Int {
        ...
        for (sequence in tlv.list!!.tlvSequence) {
            ...
            if (objectIdentifier.startsWith(PACE_OID)) {
                ...
                throw IllegalArgumentException(UNKNOWN_SECURITY_INFO)
            }
        }
