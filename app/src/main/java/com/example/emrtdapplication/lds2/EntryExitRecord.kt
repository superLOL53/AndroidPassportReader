 ...
 * @property certificateReference The reference record number
 * to a certificate in the certificate store
 * @throws IllegalArgumentException If any mandatory field is
 * missing/invalid or if a tag is invalid
 */
class EntryExitRecord(val record: TLVSequence, val recordNumber: Byte) {
    ...
    init {
        ...
        if (record.tlvSequence.size != TRAVEL_RECORD_SIZE) {
            throw IllegalArgumentException(INVALID_RECORD_SIZE)
        }
        for (tlv in record.tlvSequence) {
            if (tlv.tag.size == 1) {
                if (tlv.tag[0] != SIGNED_INFO_TRAVEL_RECORD || tlv.list == null) {
                    throw IllegalArgumentException(INVALID_SIGNED_INFO_TAG)
                }
                ...
                for (t in tlv.list!!.tlvSequence) {
                    ...
                    when (t.tag[1]) {
                        ...
                        DATE ->
                            date = t.value!!.decodeToString()
                        INSPECTION_AUTHORITY ->
                            inspectionAuthority = t.value!!.
                                decodeToString().
                                replace(FILLER_CHARACTER, ' ')
                        INSPECTION_LOCATION ->
                            inspectionLocation = t.value!!.
                                decodeToString().
                                replace(FILLER_CHARACTER, ' ')
                        INSPECTOR_REFERENCE ->
                            inspectorReference = t.value!!.
                                decodeToString().
                                replace(FILLER_CHARACTER, ' ')
                        INSPECTION_RESULT ->
                            inspectionResult = t.value!!.
                                decodeToString().
                                replace(FILLER_CHARACTER, ' ')
                        TRAVEL_MODE ->
                            travelMode = when (t.value!![0]) {
                                TRAVEL_MODE_AIR_BYTE -> "Air"
                                TRAVEL_MODE_SEA_BYTE -> "Sea"
                                TRAVEL_MODE_LAND_BYTE -> "Land"
                                else -> null
                            }
                        STAY_DURATION_TRAVEL_RECORD ->
                            stayDuration = run {
                                var tmp = 0
                                for (b in t.value!!) {
                                    tmp = (tmp shl BYTE_BIT_SIZE) + b.toUByte().toInt()
                                }
                                tmp
                            }
                        CONDITIONS -> conditions =
                            t.value!!.decodeToString().
                                replace(FILLER_CHARACTER, ' ')
                    }
                }
                ...
                if (tlv.tag[0] != TRAVEL_1 || (tlv.tag[1] != ISSUING_STATE &&
                    ...
                    throw IllegalArgumentException(INVALID_TAG)
                }
                when (tlv.tag[1]) {
        ...
        if (signedInfo == null) {
            throw IllegalArgumentException(UNSPECIFIED_SIGNED_INFORMATION)
        }
        this.signedInfo = signedInfo
        if (state1 == null || !state1.contentEquals(state2)) {
            throw IllegalArgumentException(UNSPECIFIED_STATE_ENTRIES)
        }
        this.state = state1
        this.visaStatus = visaStatus
        if (date == null) {
            throw IllegalArgumentException(UNSPECIFIED_DATE)
        }
        this.date = date
        if (inspectionAuthority == null) {
            throw IllegalArgumentException(UNSPECIFIED_INSPECTION_AUTHORITY)
        }
        this.inspectionAuthority = inspectionAuthority
        if (inspectionLocation == null) {
            throw IllegalArgumentException(UNSPECIFIED_INSPECTION_LOCATION)
        }
        this.inspectionLocation = inspectionLocation
        if (inspectorReference == null) {
            throw IllegalArgumentException(UNSPECIFIED_INSPECTOR_REFERENCE)
        }
        ...
        if (signature == null) {
            throw IllegalArgumentException(UNSPECIFIED_SIGNATURE)
        }
        this.signature = signature
        if (certificateReference == null || certificateReference.isEmpty()) {
            throw IllegalArgumentException(UNSPECIFIED_CERTIFICATE_REFERENCE)
        }
    ...
    fun verify(certificate: Certificate) {
        try {
            ...
            isVerified = Crypto.verifySignature(
                ...
                publicKey!!, signedInfo, signature)
            return
        } catch (_: Exception) {
